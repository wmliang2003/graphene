...
/*============================================================================*/
/* TODO: These should be moved to crypto adapter code */
static int recv_cb(void* ctx, uint8_t* buf, size_t len) {
    ...
    int ret = ocall_read(fd, buf, (uint32_t)len);

    if (IS_ERR(ret)) {
        if (ERRNO(ret) == EPIPE || ERRNO(ret) == ECONNRESET)
...
static int send_cb(void* ctx, uint8_t const* buf, size_t len) {
    ...
    int ret = ocall_write(fd, buf, (uint32_t)len);
    if (IS_ERR(ret)) {
        if (ERRNO(ret) == EPIPE || ERRNO(ret) == ECONNRESET)
            return MBEDTLS_ERR_NET_CONN_RESET;
        if (ERRNO(ret) == EINTR)
...
int _DkStreamSecureInit(PAL_HANDLE stream, bool is_server, PAL_SESSION_KEY* session_key,
    ...
    ret = mbedtls_ctr_drbg_seed(&ssl_ctx->ctr_drbg, mbedtls_entropy_func, &entropy, NULL, 0);
    if (ret != 0) {
        ret = -PAL_ERROR_DENIED;
        goto failed;
    }
    ...
    mbedtls_ssl_conf_rng(&ssl_ctx->conf, mbedtls_ctr_drbg_random, &ctr_drbg);

    ret = mbedtls_ssl_conf_ciphersuites(&ssl_ctx->conf, ssl_ctx->ciphersuites);
    if (ret != 0) {
        ret = -PAL_ERROR_DENIED;
        goto failed;
    }

    const uint8_t* psk  = (const uint8_t*)session_key;
    size_t psk_len      = sizeof(*session_key);
    char psk_identity[] = "dummy";

    ret = mbedtls_ssl_conf_psk(&ssl_ctx->conf, psk, psk_len, psk_identity, sizeof(psk_identity) - 1);
    if (ret != 0) {
        ret = -PAL_ERROR_DENIED;
        goto failed;
    }
    ...
    mbedtls_ssl_set_bio(&ssl_ctx->ssl, stream, send_cb, recv_cb, NULL);
